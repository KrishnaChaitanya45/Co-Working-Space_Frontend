...
function TextChannels({ isActive }: { isActive: boolean }) {
  ...
  const fetchChannels = async () => {
    if (channels && channels.textChannels && channels.textChannels.length > 0) {
      const thisServerChannels = channels.textChannels.filter(
      ...
      typeof selectedServer.server.textChannels[0] != typeof "random"
    ) {
      setFetchedChannels(selectedServer.server.textChannels);
      dispatch(addTextChannel(selectedServer.server.textChannels));
      setLoadingChannels(false);
      return;
    }
    try {
      const response = await axiosWithAccessToken.get(
        "/channel/" + selectedServer.server._id
      );
      if (response.status === 200) {
        console.log("REQUEST SUCCESSFULL", response.data.channels);

        dispatch(addTextChannel(response.data.channels));
        const updatedServer = JSON.parse(JSON.stringify(selectedServer));
        updatedServer.server.textChannels = response.data.channels;
                ...
                <motion.div
                  ...
                  onClick={() => {
                    ...
                    if (channel.restrictAccess && isUser && !userPresent) {
                      ...
                      dispatch(selectTextChannel(channel));
                    }
                  }}
                  whileTap={{
                    scale: 0.9,
