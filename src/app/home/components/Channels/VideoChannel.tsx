...
import { AppSelector, useAppDispatch } from "@/redux/hooks";
import {
  addChannels,
  addVideoChannel,
  selectServer,
...
function VideoChannel({ isActive }: { isActive: any }) {
  ...
  const fetchChannels = async () => {
    if (
      ...
      channels.videoChannels.length > 0
    ) {
      const thisServerChannels = channels.videoChannels.filter(
      ...
      typeof selectedServer.server.videoChannels[0] != typeof "random"
    ) {
      setFetchedChannels(selectedServer.server.videoChannels);
      dispatch(
        addChannels({
          videoChannels: selectedServer.server.videoChannels,
        })
      );
      setLoadingChannels(false);
      return;
    ...
    try {
      const response = await axiosWithAccessToken.get(
        "/channel/" + selectedServer.server._id + "?type=video"
      );
      if (response.status === 200) {
        console.log("REQUEST SUCCESSFULL", response.data.channels);

        dispatch(addVideoChannel(response.data.channels));
        const updatedServer = JSON.parse(JSON.stringify(selectedServer));
        updatedServer.server.videoChannels = response.data.channels;
                ...
                <motion.div
                  ...
                  onClick={() => {
                    ...
                    if (channel.restrictAccess && isUser && !userPresent) {
                      ...
                      dispatch(selectVideoChannel(channel));
                    }
                  }}
                  whileTap={{
                    scale: 0.9,
